...
@MainActor
final class KubeVirtSession: ObservableObject {
    @Published var kind: KubeVirtKind = .virtualMachine
    @Published var scope: WorkloadNamespaceScope = .all
    @Published var filter = ""
    @Published private(set) var namespaces: [String] = []
    @Published private(set) var rows: [KubeVirtRow] = []
    @Published private(set) var lastError: String?
    @Published private(set) var isRefreshing = false

    var kubeconfigURL: URL
    private let injected: (any KubeVirtClient)?
    private var pollTask: Task<Void, Never>?

    init(client: any KubeVirtClient) {
    ...
    func makeClient() -> any KubeVirtClient {
        injected ?? SwiftkubeKubeVirtClient(kubeconfigURL: kubeconfigURL)
    }
}
