...
struct HostPathsTests {
    @Test func applicationSupportHoldsSettingsEngineKubeconfigAndVM() {
        ...
        #expect(paths.osImageLock.lastPathComponent == "os.img.lock")
        #expect(paths.dataImageLock.lastPathComponent == "data.img.lock")
        #expect(paths.diskLockURLs == [paths.osImageLock, paths.dataImageLock])
